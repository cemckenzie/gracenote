        ...
        def gnsdk_init license_path, client_id, client_id_tag, user_path, user_type
            #initialize manager
                ...
                if File.file?( license_path ) 
                    $error_gnsdk = GnSDK::gnsdk_manager_initialize($managr_handle, license_path, GnSDK::GNSDK_MANAGER_LICENSEDATA_FILENAME)
                ...
                if $user_handle == nil
                    ...
                    # save the user
                    userfile = File.open(user_path, "w")
